...
function buildModelDescription(model: string, configHints: CodexConfigHints): string {
  ...
  if (sessionId) {
    ...
    if (sessionModel && isCodexModelId(sessionModel.modelId)) {
      return {
        ...
        source: 'session',
        reasoningEffort: normalizeCodexReasoningEffort(sessionModel.variant ?? undefined),
      }
    }
  }
  ...
  if (channelId) {
    ...
    if (channelModel && isCodexModelId(channelModel.modelId)) {
      return {
        ...
        source: 'channel',
        reasoningEffort: normalizeCodexReasoningEffort(channelModel.variant ?? undefined),
      }
    }
  }
