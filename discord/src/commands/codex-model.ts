...
import { getKimakiMetadata, resolveTextChannel } from '../discord-utils.js'
...
export async function handleCodexModelCommand({
  ...
  if (isThread) {
    ...
    await interaction.editReply(
      'This command can only be used in project channels or threads',
    )
    return
  }
...
export async function handleCodexModelEffortSelectMenu(
  ...
  await interaction.editReply({
    content:
      `Selected \`${context.selectedModelId}\` / \`${selectedEffort}\`.\nApply it to:`,
    components: [
      new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu),
    ],
  })
...
export async function handleCodexUnsetModelCommand({
  ...
  if (isThread) {
    ...
    if (!textChannel) {
      await interaction.editReply(
        'Could not resolve the parent project channel',
      )
      return
    }
  ...
  if (
    ...
    await interaction.editReply(
      'Cleared the Codex model override for this thread.',
    )
    return
  }
  ...
  if (channelModel?.modelId.startsWith('codex/')) {
    ...
    await interaction.editReply(
      'Cleared the Codex model override for this channel.',
    )
    return
  }
